...
def get_command(message):
    ...
    if message.text == "/start":
        anime_bot.send_message(message.from_user.id,
                               'はじめまして! In Japanese it means: '
                               '"Nice to meet you"! \n This bot can help you '
                               'to choose an anime to watch, a manga to read '
                               'or find a nice quote from them. \nPlease, '
                               'write "/help" - if you need help, "/quote" - '
                               'if you want to find a random anime quote,'
                               '"/anime - if you want to find a random anime, '
                               '"/manga" - if you want to find a random '
                               'manga, /character - if you want to discover '
                               'any profile or /schedule - '
                               'if you want to get a schedule for '
                               'a particular day. Have a good day!')
    elif message.text == "/help":
        anime_bot.send_message(message.from_user.id,
                               'Please, write /help" - if you '
                               'need help, "/quote" - if you want to find a '
                               'random anime quote, "/anime - if you want '
                               'to find a random anime, "/manga" - '
                               'if you want to find a random manga, '
                               '/character - if you want to discover '
                               'any profile or /schedule - if you want to'
                               'get a schedule for a particular day. '
                               'If nothing happened, do not worry, bot '
                               'can think for a few seconds. Have a good day!')
    elif message.text == "/anime":
        anime_id = randrange(0, 10000)
        url = requests.get("https://myanimelist.net/anime/" +
                           str(anime_id) + "/")
        ...
        for elem in anime:
            if elem == "title":
                anime_description += \
                    ("The title: " + str(anime[elem]) + ". \n")
            if elem == "title_japanese":
                anime_description += \
                    ("The original title: " + str(anime[elem]) + ". \n")
            if elem == "status":
                anime_description += \
                    ("The status: " + str(anime[elem]) + ". \n")
            if elem == "type":
                anime_description += \
                    ("The type: " + str(anime[elem]) + ". ")
            if elem == "episodes":
                anime_description += \
                    ("The amount of episodes: " + str(anime[elem]) + ". \n")
            if elem == "rating":
                anime_description += \
                    ("The rating: " + str(anime[elem]) + ". \n")
            if elem == "score":
                anime_description += \
                    ("The score: " + str(anime[elem]) + ". \n")
            if elem == "synopsis":
                anime_description += \
                    ("The synopsis: " + str(anime[elem]) + ". \n")
            if elem == "premiered":
                anime_description += \
                    ("The anime was premiered: " + str(anime[elem]) + ". \n")
            if elem == "genres":
                genre = ""
        ...
        for elem in manga:
            if elem == "title":
                manga_description += (
                        "The title: " + str(manga[elem]) + ". \n")
            if elem == "title_japanese":
                manga_description += (
                        "The original title: " + str(manga[elem]) + ". \n")
            if elem == "status":
                manga_description += (
                        "The status: " + str(manga[elem]) + ". \n")
            if elem == "volumes":
                manga_description += ("The amount of volumes: " + str(
                    manga[elem]) + ". \n")
            if elem == "chapters":
                manga_description += ("The amount of chapters: " + str(
                    manga[elem]) + ". \n")
            if elem == "score":
                manga_description += (
                        "The score: " + str(manga[elem]) + ". \n")
            if elem == "synopsis":
                manga_description += (
                        "The synopsis: " + str(manga[elem]) + ". \n")
        manga_description += (
                "\nRead more on " + "https://myanimelist.net/manga/" +
                str(manga_id) + "/")
        ...
        url = requests.get(
            "https://myanimelist.net/character/" + str(character_id) + "/")
        while url.status_code == 404:
            character_id = randrange(0, 10000)
            url = requests.get(
                "https://myanimelist.net/character/" + str(character_id) + "/")
        ...
        for elem in character:
            if elem == "name":
                character_description += (
                        "Name: " + str(character[elem]) + ".\n")
            if elem == "name_kanji":
                character_description += ("Name written in kanji: " + str(
                    character[elem]) + ".\n")
            if elem == "about":
                character_description += (
                        ...
                        str(character[elem]) + ".\n")
        character_description \
            += "\nRead more on " + "https://myanimelist.net/character/" + \
               str(character_id) + "/"
        anime_bot.send_message(message.from_user.id, character_description)
    elif message.text == "/schedule":
        schedule_text = "Please, write the day of the week " \
                        "(for example: 'monday') and then you will " \
                        "get the schedule of anime for that day."
        ...
        for i in range(len(schedule["monday"])):
            schedule_monday += (str(i + 1) + ". " + schedule["monday"][i][
                "title"] + "\n")
        ...
        for i in range(len(schedule["tuesday"])):
            schedule_tuesday += (str(i + 1) + ". " + schedule["tuesday"][i][
                "title"] + "\n")
        ...
        for i in range(len(schedule["wednesday"])):
            schedule_wednesday += (str(i + 1) + ". " +
                                   schedule["wednesday"][i]["title"] + "\n")
        ...
        for i in range(len(schedule["thursday"])):
            schedule_thursday += (str(i + 1) + ". " +
                                  schedule["thursday"][i]["title"] + "\n")
        ...
        for i in range(len(schedule["friday"])):
            schedule_friday += (str(i + 1) + ". " +
                                schedule["friday"][i]["title"] + "\n")
        ...
        for i in range(len(schedule["saturday"])):
            schedule_saturday += (str(i + 1) + ". " +
                                  schedule["saturday"][i]["title"] + "\n")
        ...
        for i in range(len(schedule["sunday"])):
            schedule_sunday += (str(i + 1) + ". " +
                                schedule["sunday"][i]["title"] + "\n")
